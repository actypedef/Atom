...
#pragma unroll
  for(int k = 0; k < KEEPER; k += KEEPER_BLOCK_K){
    ...
    //- Load to SMEM until all keepers are loaded
    bool predGuard = (k + (STAGE - 1) * KEEPER_BLOCK_K) < KEEPER;
    loadASMem(
      shmem + writePtr * shmem_stage_offset,
      A_keeper + sizeof(int8_t) * (bj * BLOCK_M * KEEPER + k + (STAGE - 1) * KEEPER_BLOCK_K),
      M_GLOBAL,
      KEEPER * 2,
      (k + (STAGE - 1) * KEEPER_BLOCK_K),
      predGuard
    );
    loadBSMem(
      shmem + shmem_B_offset + writePtr * shmem_stage_offset,
      B_keeper + sizeof(int8_t) * (bi * BLOCK_N * KEEPER + k + (STAGE - 1) * KEEPER_BLOCK_K),
      KEEPER * 2,
      (k + (STAGE - 1) * KEEPER_BLOCK_K),
      predGuard
    );
    ...
    //- dequant and accumulate in mainloop
    dequant(
      c,
      a_s,
      b_s,
