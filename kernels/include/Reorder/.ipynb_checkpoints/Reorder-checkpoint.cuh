...
__global__ void reorder_fp16_i4_kernel(
  ...
  __shared__ float maxv4bit;
  __shared__ float maxv8bit;
  ...
  s8out = s8out + row_id * KEEPER_SIZE;
  s4out = s4out + row_id * (HIDDEN_DIM - KEEPER_SIZE) / 2; // Pack two int4 into one int8
  ...
  if (ty >= bdy - KEEPER_SIZE / GROUP_SIZE) maxv4bit = mymax(maxv4bit, maxv);
  else maxv8bit = mymax(maxv8bit, maxv);
  cta.sync();

  if (ty >= bdy - KEEPER_SIZE / GROUP_SIZE) maxv = maxv4bit;
  else maxv = maxv8bit;

  // Calculate scales
  // Specific layout
  int replicated_row_id = scale_index(row_id);
  if (ty >= bdy - KEEPER_SIZE / GROUP_SIZE) {//-
    // automatically choose the kept channels group
    ...
    if(tx == 0){
      UNROLL
      for(int j = 0; j < 4; ++j){
        s8scale[(KEEPER_SIZE / GROUP_SIZE - bdy + ty) * scale_ldm + replicated_row_id + 2 * j] = (half) maxv;
      }
    }
  } else {
    // INT4 quantize
  ...
  int lower_bound = (ty >= bdy - KEEPER_SIZE / GROUP_SIZE) ? -128 : -8;
  int upper_bound = (ty >= bdy - KEEPER_SIZE / GROUP_SIZE) ? 127 : 7;
  ...
  if(ty >= bdy - KEEPER_SIZE / GROUP_SIZE){//-
    ...
    s8out_float4[tx * 2 + (KEEPER_SIZE / GROUP_SIZE - bdy + ty) * bdx * 2 + 0] = input_frag_float4[0];
    s8out_float4[tx * 2 + (KEEPER_SIZE / GROUP_SIZE - bdy + ty) * bdx * 2 + 1] = input_frag_float4[1];
  }else{
...
void run_reorder_fp16_i4(
  ...
  static_assert(group_size % 128 == 0 && hidden_dim == 4096, "Current only support 128x4096.");
  dim3 grids(seq_len);
  dim3 blocks(4, 32);
